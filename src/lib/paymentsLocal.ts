...
export function getStudentFeesMap(): FeesMap {
  const f = read<any>(KEYS.feesPerClass, {});
  if (f && typeof f === "object") return f as FeesMap;
  return {};
}
...
export function setFeesAnnexes(list: FraisAnnexe[]) {
  write<FraisAnnexe[]>(KEYS.frais, list);
}
...
export function setServices(list: Service[]) {
  write<Service[]>(KEYS.services, list);
}
